...
Future<void> sendBottleValve9StateToFlask(
    String control, bool state, BuildContext context) async {
  try {
    final url = Uri.parse(
        'http://10.0.2.2:5000/bottle_valve9'); // Update the URL to match your Flask endpoint
    ...
    if (response.statusCode == 200) {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Bottle Valve 9 control updated successfully')),
        );
      }
    } else {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Failed to update Bottle Valve 9 control')),
        );
      }
    }
  } catch (e) {
