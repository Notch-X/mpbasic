...
Future<void> sendV19ValveStateToFlask(
    String control, bool state, BuildContext context) async {
  try {
    final url = Uri.parse(
        'http://10.0.2.2:5000/v19_valve'); // Update the URL to match your Flask endpoint
    ...
    if (response.statusCode == 200) {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('V19 Valve control updated successfully')),
        );
      }
    } else {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to update V19 Valve control')),
        );
      }
    }
  } catch (e) {
